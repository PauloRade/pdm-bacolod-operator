...
[ExecuteAlways]
public class MeshGradientHollowRoundedBorder : MonoBehaviour
{
    ...
    void Update()
    {
        if (topLeftRadius != lastTL || topRightRadius != lastTR ||
            ...
            hollow != lastHollow || seed != lastSeed)
        {
            GenerateMesh();
            StoreLastValues();
        }
        ...
        for (int px = 0; px < width; px++)
        {
            for (int py = 0; py < height; py++)
            {
                ...
                if (px < tlPx && py >= height - tlPx)
                {
                    ...
                    if (dx * dx + dy * dy > tlPx * tlPx) insideShape = false;
                }
                if (px >= width - trPx && py >= height - trPx)
                {
                    float dx = px - (width - trPx);
                    float dy = py - (height - trPx);
                    if (dx * dx + dy * dy > trPx * trPx) insideShape = false;
                }
                if (px < blPx && py < blPx)
                {
                    float dx = blPx - px - 1;
                    float dy = blPx - py - 1;
                    if (dx * dx + dy * dy > blPx * blPx) insideShape = false;
                }
                if (px >= width - brPx && py < brPx)
                {
                    float dx = px - (width - brPx);
                    float dy = brPx - py - 1;
                    if (dx * dx + dy * dy > brPx * brPx) insideShape = false;
                }
